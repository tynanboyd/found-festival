	...
	*/
	...
	private static function _step_update_files_list( $custom_root = '', $startAt = 0, $items = array() ) {
		...
		foreach( $files as $file => $ignoreID ) {
			...
			$pathed_file_varred = self::_fileToVar( $pathed_file );
			
			if ( ! isset( self::$_catalog[ $pathed_file_varred ] ) ) { // File not already in signature list. Add it in with initial values.
				if ( '' != $custom_root ) { // Was not added earlier yet.
					self::$_state['stats']['files_total_count']++;
				}
				self::$_catalog[ $pathed_file_varred ] = self::$_signatureDefaults;
				$filesAdded++;
				...
				if ( ( pb_backupbuddy::$options['log_level'] == '3' ) || ( $addedSinceOutput > $outputEvery ) ) { // Full logging enabled.
					pb_backupbuddy::status( 'details', 'Added `' . $addedSinceOutput . '` more files. Last file: `' . $pathed_file_varred . '`.' );
				...
				*/
				...
				if ( '3' == pb_backupbuddy::$options['log_level'] ) { // Full logging enabled.
					pb_backupbuddy::status( 'details', 'Add to catalog: `' . $pathed_file_varred . '`.' );
				}
			} else { // Already exists in catalog.
				if ( 0 == self::$_catalog[ $pathed_file_varred ]['b'] ) { // File not backed up to server yet (pending send).
					if ( !isset( self::$_catalog[ $pathed_file_varred ]['d'] ) || ( true !== self::$_catalog[ $pathed_file_varred ]['d'] ) ) { // Not pending deletion already.
						if ( '' == $custom_root ) { // Only increment existing files if scanning from root (because stats were reset for fresh count).
							self::$_state['stats']['files_pending_send']++;
						}
					}
				} else { // Local file already exists in catalog and on server. Make sure not marked for deletion.
					if ( isset( self::$_catalog[ $pathed_file_varred ]['d'] ) && ( true === self::$_catalog[ $pathed_file_varred ]['d'] ) ) { // Was marked to delete. Remove deltion mark BUT do rescan in case this is a new version of the file since it was for some reason marked to delete.
						unset( self::$_catalog[ $pathed_file_varred ]['d'] ); // Don't immediately delete. (unset to save mem)
						self::$_catalog[ $pathed_file_varred ]['r'] = 0; // Reset last scan time so it gets re-checked.
					}
				}
			}
		}
		...
		foreach( self::$_catalog as $signatureFile => &$signatureDetails ) {
			...
			$signatureFileUnvarred = self::_varToFile( $signatureFile );
			...
			if ( '' != $custom_root ) { // Custom root. Ignore removing any files not within the custom root since we did not scan those so they are not in the $files array.
				if ( $root != substr( $signatureFileUnvarred, 0, $root_len ) ) { // Beginning of filename does not match root so not applicable for this scan. Skip.
					continue;
				}
			}
			if ( ! isset( $files[ $signatureFileUnvarred ] ) ) { // File no longer exists in new scan. Mark for deletion.
				//$sinceLogTrim++;
				$filesDeleted++;
	...
	private static function _step_update_files_signatures( $startAt = 0 ) {
		...
		foreach( self::$_catalog as $signatureFile => &$signatureDetails ) {
			...
			if ( ! file_exists( ABSPATH . $signatureFileUnvarred ) ) { // File has been deleted.
				$filesDeleted++;
				$signatureDetails['d'] = true;
				self::$_state['stats']['files_pending_delete']++;
			...
			$stat = @stat( ABSPATH . $signatureFileUnvarred );
			if ( false === $stat ) {
				$error = 'Unable to retrieve stat() for file `' . $signatureFileUnvarred . '`. Check file permissions.';
				backupbuddy_core::addNotification( 'live_error', 'BackupBuddy Stash Live Error', $error );
			...
			if ( 0 == $signatureDetails['m'] ) {
				pb_backupbuddy::status( 'error', 'Error #8438934: File modified time unexpectly zero (0). File permissions may be blocking proper modification detection on file `' . $signatureFileUnvarred  . '`.' );
			}
	...
	private static function _step_process_file_deletions( $startAt = 0 ) {
		...
		foreach( self::$_catalog as $signatureFile => &$signatureDetails ) {
			...
			// Cancel any in-process remote sends of deleted files.
			$sendFileoptions = glob( backupbuddy_core::getLogDirectory() . 'fileoptions/send-live_' . md5( $signatureFileUnvarred ) . '-*.txt' );
			if ( ! is_array( $sendFileoptions ) ) {
				$sendFileoptions  = array();
			}
			foreach( $sendFileoptions as $sendFileoption ) {
				...
				if ( true !== ( $result = $fileoptions_obj->is_ok() ) ) {
					pb_backupbuddy::status( 'error', __('Error #9034.32393. Unable to access fileoptions data related to file `' . $signatureFileUnvarred  . '`. Skipping cleanup of file send.', 'it-l10n-backupbuddy' ) . ' Error: ' . $result );
					continue;
				}
				// Something wrong with fileoptions. Let cleanup handle it later.
				if ( ! isset( $fileoptions_obj->options['status'] ) ) {
				...
				$fileoptions_obj->options['finish_time'] = -1;
				$fileoptions_obj->save();
				pb_backupbuddy::status( 'details', 'Cancelled in-progress send of deleted file `' . $signatureFileUnvarred . '`.' );
				unset( $fileoptions_obj );
			}
			
			// If file has been backed up to server then we need to QUEUE the file for deletion.
			if ( 0 != $signatureDetails['b'] ) {
				$deleteQueue[] = $signatureFileUnvarred;
	...
	private static function _process_internal_delete_queue( $destination_settings, &$deleteQueue, &$filesDeleted ) {
		...
		if ( true !== ( $delete_result = pb_backupbuddy_destination_live::deleteFiles( $destination_settings, $deleteQueue ) ) ) {
			...
			foreach( $deleteQueue as $signatureFile ) {
				...
				unset( self::$_catalog[ self::_fileToVar( $signatureFile ) ] );
			}
	...
	// Calculates simple string replacement of common paths with short variables to reduce catalog file and memory size usage.
